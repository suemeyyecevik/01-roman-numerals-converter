...
@app.route("/")
def index():
    return render_template("index.html",developer_name='sumeyye-Instructor')
...
def convert_html():
    if request.method=="POST":
        ...
        if int(number1)<1 or not number1.isdigit() or int(number1)>3999:
            return render_template("convert.html", developer_name='Sumeyye-Instructor')
        else:
            return render_template("convert.html", developer_name='Sumeyye-Instructor', convert=convert(number1),number=number1)
    else:
